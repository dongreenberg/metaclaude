import type { Session, TrackedCommand } from "./daemon.js";
...
function formatCommand(cmd: TrackedCommand): string {
  const exitIndicator =
    cmd.exitCode === null ? "" : cmd.exitCode === 0 ? "" : ` (exit ${cmd.exitCode})`;
...
export function generateContext(session: Session): string {
