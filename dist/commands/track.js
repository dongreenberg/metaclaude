...
function sendCommand(sessionId, command, exitCode, cwd, terminalId) {
...
export async function track(sessionId, terminalId) {
...
`;
    ...
    const tmpRcFile = path.join(os.tmpdir(), `metaclaude-rc-${Date.now()}.${shellName}rc`);
    // Read existing rc file and append our content
    const homeRc = path.join(os.homedir(), shellName === "zsh" ? ".zshrc" : ".bashrc");
    let existingRc = "";
    ...
    child.on("exit", (code) => {
        ...
        try {
            if (shellName === "zsh") {
                ...
                if (fs.existsSync(tmpRcFile))
                    fs.unlinkSync(tmpRcFile);
            }
        }
        catch {
