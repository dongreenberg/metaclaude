...
`;
    fs.writeFileSync(path.join(hooksDir, "metaclaude-session-start.js"), sessionStartScript);
    fs.chmodSync(path.join(hooksDir, "metaclaude-session-start.js"), "755");
    fs.writeFileSync(path.join(hooksDir, "metaclaude-inject.js"), userPromptSubmitScript);
    fs.chmodSync(path.join(hooksDir, "metaclaude-inject.js"), "755");
}
const METACLAUDE_HOOK_MARKER = "metaclaude";
function addMetaclaudeHooks(settings, hooksDir) {
    if (!settings.hooks) {
        settings.hooks = {};
    }
    ...
    if (settings.hooks.SessionStart) {
        settings.hooks.SessionStart = settings.hooks.SessionStart.filter((h) => !h.hooks.some((hh) => hh.command.includes(METACLAUDE_HOOK_MARKER)));
    }
    if (settings.hooks.UserPromptSubmit) {
        settings.hooks.UserPromptSubmit = settings.hooks.UserPromptSubmit.filter((h) => !h.hooks.some((hh) => hh.command.includes(METACLAUDE_HOOK_MARKER)));
    }
    // Add new hooks
...
function removeMetaclaudeHooks(settings) {
    if (!settings.hooks) {
        return settings;
    }
    if (settings.hooks.SessionStart) {
...
export function uninstall(global) {
    const settingsPath = getSettingsPath(global);
    const hooksDir = getHooksDir(global);
    const scope = global ? "user" : "project";
